...
export default function PlayerClient({ username }: PlayerClientProps) {
  ...
  useEffect(() => {
    const fetchRank = async () => {
      if (player && player.tiers[activeTab as keyof typeof player.tiers]) {
        const rank = await getPlayerRank(activeTab, player.tiers[activeTab as keyof typeof player.tiers]);
        setPlayerRank(rank);
      }
    };
